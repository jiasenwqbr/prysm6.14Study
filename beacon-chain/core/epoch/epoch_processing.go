...
//	 for index, validat or in enumerate(state.validators):
...
func ProcessRegistryUpdates(ctx context.Context, st state.BeaconState) (state.BeaconState, error) {
	...
	// 步骤1: 扫描所有验证者，分类收集
	// 使用 ReadFromEveryValidator 回调，避免复制整个验证者集
	// 收集三类验证者：
	// 1. eligibleForActivationQ: 可进入激活队列
	// 2. eligibleForEjection: 应被强制退出
	// 3. eligibleForActivation: 可从队列中激活
